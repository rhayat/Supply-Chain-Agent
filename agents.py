...
class Wholesaler(BaseAgent):
    ...
    def forward(self, incoming_message: str, sender_role: str = "Retailer") -> str:
        ...
        # 2. GENERATE UNIQUE SALT TO BUST CACHE
        run_id = f"{datetime.datetime.now().isoformat()}-{random.randint(10000,99999)}"

        context_str = (
            f"[[SYSTEM_RUN_ID: {run_id}]]\n"  # <--- Unique String prevents caching
